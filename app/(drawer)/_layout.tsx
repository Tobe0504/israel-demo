  ...
  return (
    <AuthContextProvider>
      <Drawer
        ...
        screenOptions={({ route }) => ({
          header: () => {
            if (route.name === "dashboard") { return <Header />; }
            else if (route.name === "sign-in") { return <AuthHeader />; }
            else if (route.name === "my-account" || route.name === "my-list" || route.name === "shipping-options") {
              return (
                <ActiveUserHeader
                  title={
                    route?.name === "my-list"
